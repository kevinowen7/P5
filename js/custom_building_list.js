	...
	dbHiddenRef.on('value', function(snapshot) {
		if (!snapshot.hasChildren()) { //no building exist in database
			...
			dbHiddenRef.on('child_added', function(snapshot) {
				...
				var address3 = snapshot.child("address_province").val();
				var address4 = snapshot.child("address_zipcode").val();
				table.row.add(["<center>"+builder[1]+"</center>",address1+" [HIDDEN]",address2,address3,address4,"<button class='btn btn-xs btn-yellow' title='Show Building' onClick='showBuild("+builder[1]+")'><i class='fa fa-star'></i></button>"]).node().id = 'build'+builder[1];
				table.draw();
				//stop loading icon
				$("#cover-spin").fadeOut(250, function() {
					$(this).hide();
